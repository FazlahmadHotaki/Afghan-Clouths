(function(){
  ...
  function productCardHTML(p){
    const offBadge = p.off > 0 ? `<span class="off">${p.off.toLocaleString('fa-IR')}٪</span>` : '';
    const wasPrice = p.oldPrice ? `<span class="was">${toman(p.oldPrice)}</span>` : '';
    return `
      <div class="p-card">
        <div class="media">
          ${offBadge}
  ...
  addForm.addEventListener('submit', e => {
    ...
    renderRail('rail' + railKey.charAt(0).toUpperCase() + railKey.slice(1), products[railKey]);

    // scroll to the rail so user sees it
    const railEl = document.getElementById('rail' + railKey.charAt(0).toUpperCase() + railKey.slice(1));
    if(railEl){
